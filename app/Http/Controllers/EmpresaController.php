...
class EmpresaController extends VoyagerBaseController
{
    ...
    public function show(Request $request, $id)
    {
        ...
        // Para QR
        $dato1 = $dataTypeContent->user_id;
        $dato2 = $dataTypeContent->nro_certificado;
        $datoqr = 'Registro de Empresas:'.$dato1.',  Certificado Nro.:'.$dato2;

        // DESDE AQUÍ MODIFICACION DE FECHAS
        // En App\Providers en la funcion boot (Para poner en español el mes)
        //Carbon::setLocale(config('app.locale'));
        //setlocale(LC_ALL, 'es_MX', 'es', 'ES', 'es_MX.utf8');
        Carbon::setLocale(config('app.locale'));
        setlocale(LC_ALL, 'en_US', 'en', 'ES', 'es_BO.UTF-8');
        $fecha_1 = Carbon::parse($dataTypeContent->fecha_fundacion)->formatLocalized('%d');
        $fecha_1m = Carbon::parse($dataTypeContent->fecha_fundacion)->format('M');
        $fecha_1a = Carbon::parse($dataTypeContent->fecha_fundacion)->formatLocalized('%Y');
        $fecha_2 = Carbon::parse($dataTypeContent->vigente_desde)->formatLocalized('%d');
        $fecha_2m = Carbon::parse($dataTypeContent->vigente_desde)->format('M');
        $fecha_2aa = Carbon::parse($dataTypeContent->vigente_desde)->format('y');
        $fecha_2a = Carbon::parse($dataTypeContent->vigente_desde)->formatLocalized('%Y');
        
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted','datoqr','fecha_1','fecha_1m', 'fecha_1a', 'fecha_2', 'fecha_2m','fecha_2aa', 'fecha_2a'));
    }
