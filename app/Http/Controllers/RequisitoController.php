...
class RequisitoController extends VoyagerBaseController
{
...
//***************************************
// Desde Aquí el calculo de vigencia
//****************************************
        $fecha_actual = Carbon::now();
        $fecha_vigencia = Carbon::parse($dataTypeContent->vencimiento_registro);
        $fecha_vigencia1 = Carbon::parse($dataTypeContent->vencimiento_lic);
        $fecha_vigencia2 = Carbon::parse($dataTypeContent->vencimiento_seprec);
        $fecha_vigencia3 = Carbon::parse($dataTypeContent->vencimiento_poliza);
        
        if($fecha_actual < $fecha_vigencia){
            $dias_reg = $fecha_vigencia->diffInDays($fecha_actual);
        }
        else{
            $dias_reg = 0-1;
        }
        
        if($fecha_actual < $fecha_vigencia1){
            $dias_lic = $fecha_vigencia1->diffInDays($fecha_actual);
        }
        else{
            $dias_lic = 0-1;
        }

        if($fecha_actual < $fecha_vigencia2){
            $dias_seprec = $fecha_vigencia2->diffInDays($fecha_actual);
        }
        else{
            $dias_seprec = 0-1;
        }

        if($fecha_actual < $fecha_vigencia3){
            $dias_poliza = $fecha_vigencia3->diffInDays($fecha_actual);
        }
        else{
            $dias_poliza = 0-1;
        }
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted', 'dias_reg', 'dias_lic', 'dias_seprec', 'dias_poliza')); 
    }
